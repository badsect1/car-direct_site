...
loadEnv(path.join(rootDir, '..', 'saehanccm.com(githup 배포)', '.env'));
...
- 주요 비교 손보사: ${randomInsurer} (현대해상, DB손보, KB손보 등 7개 손해보험사)
...
4. **본문 HTML(contentHtml)**:
   ...
   - 본문 중간에 자연스럽게 1분 최저가 비교견적 CTA 배너 배치:
     <div class="intext-cta-banner"><h4>내 차 다이렉트 자동차보험료, 1분 만에 최저가 비교해보세요</h4><p>국내 주요 7대 손해보험사(현대해상, DB, KB, AXA, 한화 등)의 특약 할인율을 한 번에 계산해 드립니다.</p><a href="${siteConfig.ctaUrl}" target="_blank" rel="noopener noreferrer" class="btn-cta-large">⚡ 1분 최저가 비교견적 조회하기</a></div>
...
async function generateArticle() {
  ...
  // slug 정제 및 중복 방지
  let slug = (postData.id || `car-insurance-guide-${Date.now()}`).toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
  ...
  const jsonLdData = [
    {
      ...
      "publisher": {
        ...
        "logo": {
          "@type": "ImageObject",
          "url": "https://car-direct.kr/wp-content/uploads/2025/02/다이렉트자동차로고1.png"
        }
      },
      "mainEntityOfPage": {
        "@type": "WebPage",
...
<head>
  ...
  <title>${postData.title} | ${siteConfig.siteName}</title>
  ...
  <meta property="og:image" content="https://car-direct.kr/wp-content/uploads/2025/02/main_1896_758.jpg" />
...
<body>
  ...
  <header class="site-header">
    <div class="header-container">
      <a href="/" class="brand-logo" title="${siteConfig.siteName} 메인으로 이동">
        <span class="brand-badge">비교견적</span>
        <span class="brand-name">${siteConfig.brandShort}</span>
  ...
  <article class="article-container">
    ...
    <header class="article-header">
      ...
      <div class="article-meta-info">
        <span>✍️ 다이렉트 자동차보험 다나와 리서치팀</span>
    ...
    <section class="article-body">
      ...
      <div class="cta-banner-box">
        ...
        <p>현대해상, DB손해보험, KB손해보험, AXA 등 주요 손해보험사 특약 할인을 실시간으로 비교해 드립니다.</p>
      ...
      <div class="compliance-box">
        <p>※ 본 광고는 소정의 광고비(원고료 등)를 지급받아 작성하였습니다.</p>
        <p>※ 보험계약 체결 전 반드시 상품설명서 및 약관을 확인하시기 바랍니다.</p>
        <p>※ 금융소비자는 해당 상품 또는 서비스에 대하여 설명을 받을 권리가 있습니다.</p>
        <p>※ 예금자보호법에 따라 예금보험공사가 보호하되, 보호한도는 본 은행에 있는 귀하의 모든 예금보호 대상 금융상품의 원금과 소정의 이자를 합하여 1인당 "최고 5천만원"이며, 5천만원을 초과하는 나머지 금액은 보호하지 않습니다.</p>
      </div>
    </section>
  </article>
  ...
  <footer class="site-footer">
    <div class="footer-container">
      <div class="footer-top">
        <div class="footer-brand">${siteConfig.siteName}</div>
        <div>제휴사: 현대해상 · DB손해보험 · KB손해보험 · AXA손해보험 · 한화손해보험 · 흥국화재 · 하나손해보험</div>
      </div>
      <p>본 칼럼은 금융소비자의 합리적인 자동차보험 선택과 권익 보호를 위해 객관적인 공시자료 및 약관을 바탕으로 작성되었습니다.</p>
      <p class="footer-legal">© ${siteConfig.siteName}. All rights reserved.</p>
    </div>
  </footer>
...
function updateSitemap(posts) {
  ...
  const baseUrl = siteConfig.siteUrl || 'https://car-direct.kr';
